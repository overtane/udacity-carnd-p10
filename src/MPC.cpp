...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // The vector fg is where the cost function and vehicle model/constraints is defined.
    ...
    for (int t = 1; t < N; t++) {
      ...
      // the value of polynomial at x0
      AD<double> f0 = coeffs[0] + coeffs[1] * x0 + coeffs[2] * x0 * x0 + coeffs[3] * x0 * x0 * x0;;
      // atan of derivative of the polynomial
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // state.size() == number of model variables 
  // N == number of timesteps in the horizon
  // 2 == number of actuators (CTE and orientation error)
